"""Production V4 live ingestion orchestrator.

The MLB StatsAPI owns canonical game identity (gamePk). Odds are matched to
those games by normalized away/home team names, so The Odds API event ID never
becomes the primary game key. All snapshots remain point-in-time records.
"""
...
def run_live_sync(game_date: str | None = None):
    ...
    sync_id = begin_sync(game_date, 'mlb-statsapi+odds+weather')
    games_written = odds_written = weather_written = stats_written = results_written = 0

    try:
        # 1. Canonical games, pitchers and venues come from MLB StatsAPI.
        ...
        # 2. Odds are matched to the canonical MLB gamePk by teams.
        odds_raw = OddsAdapter(cfg.odds_base_url, cfg.odds_api_key).odds(game_date)
        game_index = {
            (_team_key(g.get('away_team_name')), _team_key(g.get('home_team_name'))): g['id']
            for g in games
        }
        ...
        odds_written = insert_odds(dedupe_odds(odds))

        # 3. Snapshot the team/pitching inputs used by the quant model.
        ...
        # 4. Persist completed scores for later settlement/calibration.
        result_rows = []
        for game in games:
            ...
            if (
                ...
                and status_text in {'final', 'game over', 'completed'}
            ):
                result_rows.append({
                    'game_id': str(game['id']),
        ...
        # 5. Weather is optional but enabled by default through Open-Meteo.
        if cfg.weather_base_url:
            ...
            for game in games:
                ...
                if lat is not None and lon is not None:
                    weather_rows.extend(
                        weather.forecast_rows(str(game['id']), float(lat), float(lon))
                    )
            if weather_rows:
